...
public final class SubstanceBlock extends ASubstanceObject {
    public SubstanceBlock(String name, boolean self, String prefix, String suffix, ICreateObject provider, IGetFaces faces, IGetModel model) {
    ...
    public static FaceProvider[] oreFaces(ASubstanceObject reference, Substance substance) {
        ...
        faces[1] = new FaceProvider(new ResourceLocation(MODID, String.join("/", "blocks", "substances", "blocks", substance.getAestheticism().getStyle(), reference.self ? substance.getName() : reference.name)), substance.getAestheticism().getOreColor());
        return faces;
    }
