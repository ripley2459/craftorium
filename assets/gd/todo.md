...
## Refonte du système de circuits - trois chaînes redondantes, une seule presque complète.

Analyse du 2026-08-14. Le mécanisme de distribution par tier est déjà excellent et ne doit pas
changer : toutes les recettes de fabrication de machines (moteur, piston, pompe, capteur, bras
robotique, et les 10 machines) consomment un tag OreDictionary générique `circuitTierN`
(`ACommonProxy.java:137-151`, `RecipesHandler.java:141-230`,
ex. `registerXXXRecipe(TIER_ONE, "circuitTier1")`) - jamais un item précis. On ne touche donc
jamais aux 50+ recettes de machines, seul l'item qui porte le tag doit être craftable.

Trois chaînes alimentent ces tags aujourd'hui :

- **Redstone** (`circuit_board_redstone` → `redstone_chip/resistor/capacitor/diode` →
  `redstone_circuit` → `advanced_redstone_circuit`, `RecipesHandler.java:30-135`) : craftable une
  fois le fix `"glue"`/`"paper"` OreDict appliqué (tags non standards, `:37,99,110,119` - à
  enregistrer sur `Items.PAPER` ou remplacer par un ingrédient interne). `circuitTier0` (porté par
  `redstone_circuit`) n'est actuellement requis par aucune machine (elles commencent à
  `circuitTier1`) - **décision : ce n'est pas un trou à combler, c'est voulu.** `circuitTier0`
  reste un palier de transition pré-électronique à part entière (mécanique/redstone, avant le
  silicium) : un chemin alternatif pour atteindre le Tier 1 sans passer par le silicium, et un tag
  OreDictionary ouvert aux autres mods (un mod tiers peut fournir/consommer `circuitTier0` sans
  dépendre de la chaîne silicium de Craftorium, et inversement). On garde le tag et
  `redstone_circuit` tel quel ; voir point 7 pour le rendre également utilisable en Tier 1.
- **Générique** (`chip`/`resistor`/`diode`/`capacitor`/`transistor` → `primitive_circuit` /
  `advanced_circuit` / `maxed_circuit`, `RecipesHandler.java:62-93`) : **mort**. Ces recettes
  consomment des items (`chip`, `resistor`, `diode`, `capacitor`) déclarés dans
  `ACommonProxy.java:64-71` mais jamais produits par aucune recette. Toute la branche est
  uncraftable.
- **Wafer/Die/Processor** (silicium → wafer → die → `microprocessor` → `processor` →
  `advanced_processor` → `mainframe`) : **presque complet**. `ram_die`/`power_die`/
  `calculus_die`/`quantum_calculus_die` se fabriquent déjà au Cutter depuis leurs wafers
  (`RecipesHandler.java:375-399`), et l'assemblage au Circuit Assembler existe déjà avec
  progression propre x4/x4 en RF (`:440-481`), fil dont le matériau vient de `Tier.getEnergy()`
  (déjà semi-réaliste). **Seul maillon manquant : aucune recette ne produit
  `ram_wafer`/`power_wafer`/`calculus_wafer`/`quantum_calculus_wafer`.**

**Plan retenu : une seule colonne vertébrale (Wafer/Die/Processor), retirer la branche
générique, garder Redstone comme palier 0 pré-électronique.**

Recettes à ajouter pour combler l'unique maillon manquant. Chaîne étendue d'un cran par rapport
à la première proposition : on sépare **découpe** (boule → wafer brut), **dopage** (wafer brut →
wafer dopé, étape intermédiaire) et **recuit/finition** (wafer dopé → wafer fini prêt à découper
en die) - plus réaliste (un wafer est dopé après découpe, pas dans le bain de silicium fondu) et
ça donne trois étapes/machines distinctes au lieu d'une seule :

  ```
  boule --(Cutter)--> wafer brut (= ram_wafer, non dopé, utilisable tel quel)
  wafer brut --(Mixer, dopage)--> doped_pn_wafer  --(Foundry, recuit)--> calculus_wafer
  wafer brut --(Mixer, dopage)--> doped_gaas_wafer --(Foundry, recuit)--> power_wafer
  wafer brut --(Mixer, dopage)--> doped_graphene_wafer --(Foundry, recuit)--> quantum_calculus_wafer
  ```

1. Nouvelles constantes `Maps.java` : `CONFIGURATION_CASTING_FORM_BOULE = 10` et
   `CONFIGURATION_CASTING_ANNEAL = 11`.
2. Foundry (`CASTING`) - couler le silicium fondu en boule (forme `BOULE`, actuellement inutilisée,
   `SubstancesObjects.java:125`) :
   ```
   CASTING.addRecipe(new MachineRecipeBuilder("casting_silicon_boule")
       .consumeFluid(LIQUID.getName(SILICON), BOULE.getAmount())
       .produceItem(BOULE.asItemStack(SILICON))
       .consumeEnergy(30000).duration(400)
       .configuration(CONFIGURATION_CASTING_FORM_BOULE).build());
   ```
3. Cutter (`CUTTING`, 1 item in / 1 item out - parfait pour une simple découpe) - boule → wafer
   brut non dopé, réutilisé tel quel comme `ram_wafer` (la mémoire n'a pas besoin de dopage) :
   ```
   CUTTING.addRecipe(new MachineRecipeBuilder("ram_wafer")
       .consumeItem(BOULE.getOre(SILICON), 1)
       .produceItem(ACommonProxy.getItemStack("ram_wafer", 4))
       .consumeEnergy(20000).duration(200).configuration(CONFIGURATION_CUTTER_CIRCUIT).build());
   ```
4. Mixer (`MIXING`, 9 items in - permet de combiner le wafer brut avec le(s) dopant(s), ce que le
   Cutter ne peut pas faire) - dopage semi-réaliste, réutilisant des substances déjà existantes
   (BORON/ARSENIC = dopage p/n réel du CMOS, GALLIUM = GaAs réel des semi-conducteurs de
   puissance, GRAPHENE = donne enfin un usage à cette substance quasi morte) :
   ```
   MIXING.addRecipe(new MachineRecipeBuilder("doped_pn_wafer")
       .consumeItem(ACommonProxy.getItemStack("ram_wafer"), 1)
       .consumeItem(DUST.asIngredient(BORON), 1)
       .consumeItem(DUST.asIngredient(ARSENIC), 1)
       .produceItem(ACommonProxy.getItemStack("doped_pn_wafer", 1))
       .consumeEnergy(40000).duration(300).configuration(CONFIGURATION_MIXING_CHEM).build());

   MIXING.addRecipe(new MachineRecipeBuilder("doped_gaas_wafer")
       .consumeItem(ACommonProxy.getItemStack("ram_wafer"), 1)
       .consumeItem(DUST.asIngredient(GALLIUM), 2)
       .produceItem(ACommonProxy.getItemStack("doped_gaas_wafer", 1))
       .consumeEnergy(35000).duration(280).configuration(CONFIGURATION_MIXING_CHEM).build());

   MIXING.addRecipe(new MachineRecipeBuilder("doped_graphene_wafer")
       .consumeItem(ACommonProxy.getItemStack("ram_wafer"), 1)
       .consumeItem(DUST.asIngredient(GRAPHENE), 2)
       .produceItem(ACommonProxy.getItemStack("doped_graphene_wafer", 1))
       .consumeEnergy(150000).duration(800).configuration(CONFIGURATION_MIXING_CHEM).build());
   ```
5. Foundry (`CASTING`, recuit/activation thermique des dopants - étape de finition réaliste avant
   découpe) - wafer dopé → wafer fini, prêt pour le Cutter :
   ```
   CASTING.addRecipe(new MachineRecipeBuilder("calculus_wafer")
       .consumeItem(ACommonProxy.getItemStack("doped_pn_wafer"), 1)
       .produceItem(ACommonProxy.getItemStack("calculus_wafer", 2))
       .consumeEnergy(50000).duration(350).configuration(CONFIGURATION_CASTING_ANNEAL).build());

   CASTING.addRecipe(new MachineRecipeBuilder("power_wafer")
       .consumeItem(ACommonProxy.getItemStack("doped_gaas_wafer"), 1)
       .produceItem(ACommonProxy.getItemStack("power_wafer", 2))
       .consumeEnergy(45000).duration(320).configuration(CONFIGURATION_CASTING_ANNEAL).build());

   CASTING.addRecipe(new MachineRecipeBuilder("quantum_calculus_wafer")
       .consumeItem(ACommonProxy.getItemStack("doped_graphene_wafer"), 1)
       .produceItem(ACommonProxy.getItemStack("quantum_calculus_wafer", 1))
       .consumeEnergy(200000).duration(1000).configuration(CONFIGURATION_CASTING_ANNEAL).build());
   ```
   (quantités/RF/durée à ajuster en playtest ; `quantum_calculus_die` reste sans consommateur en
   aval pour l'instant - cohérent avec le vaporware quantum/wetware noté plus bas, la recette de
   wafer est ajoutée par cohérence de chaîne mais rien n'en dépend encore. `doped_pn_wafer`,
   `doped_gaas_wafer`, `doped_graphene_wafer` sont 3 nouveaux items intermédiaires à déclarer dans
   `ACommonProxy.java`, même pattern que les items `circuits/*` existants.)
